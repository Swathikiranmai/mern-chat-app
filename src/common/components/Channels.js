  ...
  render() {
    ...
    const moreChannelsModal = (
      <div style={{background: 'grey'}}>
        <Modal key={2} show={this.state.moreChannelsModal} onHide={this.closeMoreChannelsModal.bind(this)}>
          <Modal.Header closeButton >
            <Modal.Title>More Channels</Modal.Title>
            <a onClick={this.createChannelWithinModal.bind(this)} style={{'cursor': 'pointer', 'color': '#85BBE9'}}>
              Create a channel
            </a>
          </Modal.Header>
          <Modal.Body>
            <ul style={{height: 'auto', margin: '0', overflowY: 'auto', padding: '0'}}>
              {restOfTheChannels.map(channel =>
                <ChannelListModalItem channel={channel} key={channel.id} onClick={this.handleChangeChannel.bind(this)} />
                )}
            </ul>
          </Modal.Body>
          <Modal.Footer>
            <button onClick={this.closeMoreChannelsModal.bind(this)}>Cancel</button>
          </Modal.Footer>
        </Modal>
      </div>
    );
    return (
      <section>
        ...
        <div>
          ...
          {moreChannelsBoolean && <a onClick={this.openMoreChannelsModal.bind(this)} style={{'cursor': 'pointer', 'color': '#85BBE9'}}> + {channels.length - 8} more...</a>}
          {moreChannelsModal}
        </div>
      </section>
    );
