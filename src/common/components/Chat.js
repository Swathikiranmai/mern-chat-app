    ...
    const mobileNav = (
      <Navbar fixedTop style={{background: '#337ab7', color: 'white'}}>
        ...
        <Navbar.Collapse style={{maxHeight: '100%'}}>
          <Button bsStyle="primary" onSelect={this.handleSignOut.bind(this)}> Sign out
          </Button>
          <section style={{order: '2', marginTop: '1.5em'}}>
            <Channels socket={socket} onClick={this.changeActiveChannel.bind(this)} channels={channels} messages={messages} dispatch={dispatch} />
          </section>
        </Navbar.Collapse>
      </Navbar>
    );
        ...
        <div className="main">
          ...
          <ul style={{wordWrap: 'break-word', margin: '0', overflowY: 'auto', padding: '0', paddingBottom: '1em', flexGrow: '1', order: '1'}} ref="messageList">
            {filteredMessages.map(message =>
              <MessageListItem handleClickOnUser={this.handleClickOnUser.bind(this)} message={message} key={message.id} />
            )}
          </ul>
